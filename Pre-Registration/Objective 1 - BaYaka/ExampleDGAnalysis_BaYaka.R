### Example code for Martin's BaYaka paper - Ordinal DG models
...
prob01 <- plogis(b01 + (log(2) * LGPUN_AVE) + (log(1.5) * LGOMNI_AVE) + 
                   ...
                   (log(0.95) * CHILDREN) + (log(0.8) * CITY) + (log(1.2) * ORDER))
...
mod <- brm(DG ~ LGPUN_AVE + LGOMNI_AVE +
             AGE + SEX + FORMALED + MAT + CHILDREN + CITY +
             ORDER,
...
df_sup0 <- as.data.frame(cbind(LGPUN_AVE = 0, LGOMNI_AVE = 0, 
                         AGE = AGE, SEX = SEX, FORMALED = FORMALED, MAT = MAT, CHILDREN = CHILDREN, CITY = CITY,
                         ORDER = ORDER))
head(df_sup0)

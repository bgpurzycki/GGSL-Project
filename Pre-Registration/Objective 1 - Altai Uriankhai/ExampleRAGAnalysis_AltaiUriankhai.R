### Example code for Attila's Altai Uriankhai paper - Binomial RAG models
### Created 29/11/2024 by Dan Major-Smith
### R version 4.4.1
...
# This is about right (37%-point increase in donating to DISTANT), as this is close to the sum of the two terms from the linear probability model above when simulating this outcome (40%-points - "(0.2 * LGPUN_AVE) + (0.2 * LGOMNI_AVE)")
...
mod_mi <- brm_multiple(RAG | trials(30) ~ LGPUN_AVE_M + LGOMNI_AVE_M +
                         AGE + SEX + FORMALED + MAT + CHILDREN +
                         ORDER + COMP,
                       family = "binomial",
                       data = imp,
...
## Model with interaction between exposures
mod_mi_int <- brm_multiple(RAG | trials(30) ~ LGPUN_AVE_M + LGOMNI_AVE_M + LGPUN_AVE_M:LGOMNI_AVE_M
                             AGE + SEX + FORMALED + MAT + CHILDREN +
                             ORDER + COMP,
                           family = "binomial",
                           data = imp,
...
for (i in 1:length(imp_all)) {
  ...
  mod_temp <- brm(RAG | trials(30) ~ LGPUN_AVE_M + LGOMNI_AVE_M +
               AGE + SEX + FORMALED + MAT + CHILDREN +
               ORDER + COMP,
             family = "binomial",
             data = df_temp,

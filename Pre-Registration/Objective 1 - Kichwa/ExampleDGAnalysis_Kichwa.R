### Example code for Kichwa paper - Ordinal DG models
...
prob01 <- plogis(b01 + (log(2) * LGPUN_AVE) + (log(1.5) * LGOMNI_AVE) + 
                   ...
                   (log(0.95) * CHILDREN) + (log(1.25) * MARKET) + (log(1.2) * ORDER))
...
### Sensitivity analysis: Inclusion of variables which could be either confounders and/or mediators - i.e., supernatural beliefs and practices regarding moralistic god (Catholic god), and Pachamama rituals. For simplicity here, will just assume that these are all mediators, rather than confounders
...
prob01 <- plogis(b01 + (log(2) * LGPUN_AVE) + (log(1.5) * LGOMNI_AVE) + 
                   (log(1.1) * AGE) + (log(0.5) * SEX) + (log(1.1) * FORMALED) + (log(0.8) * MAT) + 
                   (log(0.95) * CHILDREN) + (log(1.25) * MARKET) + (log(1.2) * ORDER) + 
                   (log(0.67) * MGPUN_AVE) + (log(0.67) * MGOMNI_AVE) + 
                   (log(1.2) * MGRIT) + (log(1.1) * LGRIT))
prob12 <- plogis(b12 + (log(2) * LGPUN_AVE) + (log(1.5) * LGOMNI_AVE) + 
                   (log(1.1) * AGE) + (log(0.5) * SEX) + (log(1.1) * FORMALED) + (log(0.8) * MAT) + 
                   (log(0.95) * CHILDREN) + (log(1.25) * MARKET) + (log(1.2) * ORDER) + 
                   (log(0.67) * MGPUN_AVE) + (log(0.67) * MGOMNI_AVE) + 
...
mod2 <- brm(DG ~ LGPUN_AVE + LGOMNI_AVE +
             AGE + SEX + FORMALED + MAT + CHILDREN + MARKET +
             ORDER +
           ...
           prior = c(prior(normal(0, 1), class = b),
                     prior(normal(0, 2), class = Intercept)),
           iter = 2000, warmup = 1000, chains = 4, cores = 4, seed = 876543)
